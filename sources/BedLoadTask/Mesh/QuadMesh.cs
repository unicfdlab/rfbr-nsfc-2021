...
namespace BedLoadTask.Mesh
{
    ...
    public class QuadMesh
    {
        ...
        public QuadMesh(TriMesh mesh)
        {
            this.mesh = mesh;
            this.Left = mesh.GetCoordsX().Min();
            this.Right = mesh.GetCoordsX().Max();
            this.Bottom = mesh.GetCoordsY().Min();
            this.Top = mesh.GetCoordsY().Max();
            x = mesh.GetCoordsX();
            y = mesh.GetCoordsY();
        }
        /// <summary>
        ...
        {
            MEM.Alloc<double>(mesh.Count, ref Zeta);
            for (uint nod = 0; nod < mesh.Count; nod++)
            {
                double xc = x[nod];
                double yc = y[nod];
                ...
                //if (flagPole[i, j] == true && flagPole[i + 1, j] == true && flagPole[i, j + 1] == true && flagPole[i + 1, j + 1] == true)
                //{
                //    double p0 = pole[i + 1][j];
                //    double p1 = pole[i + 1][j + 1];
                //    double p2 = pole[i][j + 1];
                //    double p3 = pole[i][j];

                //    double x0 = j * dx + Left;
                //    double x1 = x0 + dx;
                //    double y0 = i * dy + Bottom;
                //    double y1 = y0 + dy;
                ...
                //    Zeta[nod] = N0 * p0 + N1 * p1 + N2 * p2 + N3 * p3;
                //}
                //else
                {
                    uint count = 0;
